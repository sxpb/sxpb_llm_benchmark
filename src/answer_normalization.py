...
def normalize_integer(
    text: str, options: NormalizationOptions
) -> Tuple[bool, Union[int, str]]:
    pattern = (
        INTEGER_PATTERN_WITH_CURRENCY if options["allow_currency"] else INTEGER_PATTERN
    )
...
def normalize_number(
    ...
    pattern = (
        NUMBER_PATTERN_WITH_CURRENCY if options["allow_currency"] else NUMBER_PATTERN
    )
    match = re.search(pattern, text)
    if not match:
...
def normalize_csv_list_ordered(
    text: str, options: NormalizationOptions
    ...
    normalized_items = [
        item if options["case_sensitive"] else item.lower() for item in items
    ]
    return True, normalized_items


def normalize_csv_list_unordered(
    text: str, options: NormalizationOptions
) -> Tuple[bool, Union[List[str], str]]:
...
def compare_values(
    actual: Any, expected: Any, kind: AnswerType, options: NormalizationOptions
) -> bool:
...
def compare_answers(
    ...
    expected_success, expected_value = normalize_answer(
        expected, kind, resolved_options
    )
