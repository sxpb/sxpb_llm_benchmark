...
def generate_orders(count: int) -> Dict[str, List[Order]]:
    ...
    for i in range(count):
        ...
        for j in range(item_count):
            price = round(fake.pyfloat(min_value=9.99, max_value=199.99, right_digits=2), 2)
...
def generate_analytics_data(days: int, start_date_str: str = '2025-01-01') -> Dict[str, List[AnalyticsMetric]]:
    ...
    for i in range(days):
        ...
        views = round(base_views * weekend_multiplier + fake.random_int(min=-1000, max=3000))
...
def generate_event_logs(count: int) -> Dict[str, List[EventLog]]:
    ...
    for _ in range(count):
        ...
        log: EventLog = {
            ...
            "statusCode": fake.random_int(min=400, max=599) if has_error else fake.random_int(min=200, max=299),
        ...
        if has_error:
            log["error"] = {
                "message": fake.random_element(elements=[
...
def generate_nested_config() -> NestedConfig:
    return {
        "environment": fake.random_element(elements=['production', 'staging', 'development']),
        "version": "1.2.3",
        "database": {
...
def generate_structural_validation_fixtures() -> List[StructuralValidationFixture]:
    ...
    fixtures: List[StructuralValidationFixture] = [
        ...
        },
        {
            ...
            "data": {
                "employees": base_data["employees"] + generate_employees(3)["employees"],
            },
            "isValid": False,
        },
        {
        ...
        },
        {
            ...
            "data": {
                "employees": [
                    {k: v for k, v in emp.items() if k != 'email'} if i % 5 == 0 else emp
                    for i, emp in enumerate(base_data["employees"])
                ],
            },
            "isValid": False,
